...
def from_nx_to_ig_graph(nx_graph):
    adj_matrix = (nx.to_numpy_matrix(nx_graph) > 0).tolist()
    return ig.Graph.Adjacency(adj_matrix)


def from_ig_to_nx_graph(ig_graph):
    matrix = ig_graph.get_adjacency()
    matrix2 = np.array(matrix.data)
    return nx.from_numpy_matrix(matrix2)


def from_ig_to_nx_partition_multiplex(partition_ig):
    partition_nx = {node: com for node, com in enumerate(partition_ig)}
    return partition_nx


def from_ig_to_nx_partition_ordinary(partition_ig):
    com_number = 0
    nodes = []
    coms = []
    for item in partition_ig:
        ...
        com_number += 1
    partition_nx = {node: com for node, com in zip(nodes, coms)}
    return partition_nx
...
def SF_leiden(graph_layers_ig, runs, alpha_grid):
    ...
    for key_alpha in tqdm_notebook(partition.keys(), leave=False):
        ...
        for simulation in range(runs):
            membership, improvement = leidenalg.find_partition_multiplex(list(graph_layers_merging.values()), 
                                                               leidenalg.ModularityVertexPartition,
                                                               n_iterations=1, seed=random.randint(0, 1000))
            nx_partit = from_ig_to_nx_partition_multiplex(membership)
            partition[key_alpha].append(nx_partit)
    return partition


def EF_leiden(graph_layers_nx, runs, alpha_grid):
    partition = {alpha: [] for alpha in alpha_grid}
    for key_alpha in tqdm_notebook(partition.keys(), leave=False):
        alphas_dict = {layer_name: value for layer_name, value in zip(graph_layers_nx.keys(), key_alpha)}
        ef_graph_nx = combine_nx(graph_layers_nx, alphas_dict)
        ef_graph_ig = from_nx_to_ig_graph(ef_graph_nx)
        for simulation in range(runs):
            membership = leidenalg.find_partition(ef_graph_ig, leidenalg.ModularityVertexPartition, n_iterations=1, 
                                                  seed=random.randint(0, 1000))
            nx_partit = from_ig_to_nx_partition_ordinary(membership)
            partition[key_alpha].append(nx_partit)
    return partition


def LF_leiden(graph_layers_nx, runs, alphas_grid):
    names = graph_layers_nx.keys()
    graph_layers_ig = {name: from_nx_to_ig_graph(graph_layers_nx[name]) for name in names}
    best_partit_ig = {name: leidenalg.find_partition(graph_layers_ig[name], 
                                                     leidenalg.ModularityVertexPartition, 
                                                     n_iterations=1,
                                                     seed=random.randint(0, 1000)) 
                      for name in names}
    best_partit_nx = {name: from_ig_to_nx_partition_ordinary(best_partit_ig[name]) for name in names}
    late_graph_layers_nx = {name: late_fusion_transient_absorption(graph_layers_nx[name], best_partit_nx[name]) for name in names}
    return EF_leiden(late_graph_layers_nx, runs, alphas_grid), best_partit_nx
...
def combine_nx(graph_layers, alphas):
    '''Make a graph out of graph layers using balance parameters alpha'''
    first_element = next(iter(graph_layers.values()))
    node_list = first_element.nodes()
    adj_matrix_new = np.zeros(shape=(len(node_list), len(node_list)))
...
def compute_experiment(objective_function, partition, graph_layers_nx):
    x, y = [], []
    for alpha, runs in tqdm_notebook(partition.items(), total=len(partition), leave=False):
        ...
        if objective_function == 'ef' or objective_function == 'lf':
            G_wb = combine_nx(graph_layers_nx, alphas_dict)
            for part in tqdm_notebook(runs, total=len(runs), leave=False):
